...
1. Постраничное получение списка данных с фильтрами:
   ...
   - sort - сортировка по колонкам (см. ниже).
...
Параметр `sort` позволяет задать порядок сортировки возвращаемых записей.

Формат: `поле[:направление]`, несколько полей разделяются запятой.
- `поле` -- имя колонки таблицы (буквы, цифры и `_`).
- `направление` -- `asc` (по возрастанию) или `desc` (по убыванию); если не указано, используется `asc`.

Если параметр `sort` не задан, сортировка выполняется по первичному ключу по возрастанию.

Примеры:
```
GET <адрес>/rdm/data/v2/{refBookCode}?sort=code

GET <адрес>/rdm/data/v2/{refBookCode}?sort=code:desc

GET <адрес>/rdm/data/v2/{refBookCode}?sort=code:asc,name:desc

GET <адрес>/rdm/data/v2/{refBookCode}?filter=code=like=I4*&sort=code:asc&page=0&size=50
```
