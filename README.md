...
| rdm-sync.auto-create.refresh-comments                                 | true                  | Включает добавление комментариев для таблиц(если их нет) и колонок при каждом запуске. Значения комментариев берутся из НСИ. По умолчанию включено. Рекомендуется отключать после того как обновление комментарием произошло для старых таблиц. Для новых добавляются безусловно |
| rdm-sync.auto-create.refresh-comments.search-depth                    | 3                     | Глубина поиска комментариев, т.е число версий в рамках которых делаем поиск комментариев                                                                                                                                                                                         |
