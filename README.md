...
1. Модуль должен быть реализован на java 11 и выше, spring-boot 2 и платформе n2o-platform версии 4.4.0;
...
Бывают справочники, в которых в новых версиях поменялся тип поля, для поддержки синхронизации данных предыдущих версий и преобразования разных типов данных добавлена возможность использовать SpEL-выражения внутри xml-маппинг. Для этого необходимо в теге`field` добавить параметр `transform-expr` и в нем описать SpEL-выражение, для преобразования нужных значений полей. При описании выражения важно учитывать тип поля в БД `sys-data-type`. Вот несколько примеров SpEL выражений, которые можно использовать:
...
- ```#root.split(',').![T(Integer).parseInt(#this)].?[#this % 2 == 0][0]``` - это выражение разделяет строку на числа, отбирает четные и возвращает первое из них.

   - `#root.split(',')` - разделяет строку #root на части, используя запятую в качестве разделителя, и создает массив строк.

   - `.[T(Integer).parseInt(#this)]` - преобразует каждую часть строки в целое число.

   - .`?[#this % 2 == 0]` - фильтрует только те числа, которые являются четными (% 2 == 0).

   - `[0]`- извлекает первое четное число из отфильтрованного списка.
...
будет использоваться конкретный маппинг. Пример:
```xml
<mapping>
   <refbook code="1.2.643.5.1.13.13.99.2.183"
            unique-sys-field="id"
