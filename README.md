...
|rdm-sync.auto_create.loader.enable| -                                                |настройка, указывающая, что для автосоздания таблиц будет использоваться стандартный механизм loader-ов. Это не позволит ссылаться на созданные таблицы в ликубейз-скриптах при первом запуске приложения. Вы можете отключить данную настройку и добавить ликубейз-скрипт, в котором укажите <customChange class="ru.i_novus.ms.rdm.sync.service.init.InitCustomTaskChange"/>. Вы можете ссылаться на созданные автоматически таблицы после этого скрипта при первом запуске приложения.  /
## Описание таблиц
